import { BookOpen, FlaskConical, History, Settings, Stethoscope } from "lucide-react";
...
function SidebarItem({ icon: Icon, label, active, comingSoon, delay = 0 }: SidebarItemProps) {
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <motion.button
          ...
          className={cn(
            "text-muted-foreground relative flex h-9 w-9 items-center justify-center rounded-md transition-colors",
        ...
        >
          ...
          {active && (
            <motion.span
              ...
              className="bg-primary absolute -left-2 h-5 w-1 rounded-r-full"
      ...
      <TooltipContent side="right" className="flex items-center gap-2">
        {label}
        {comingSoon && (
          <span className="bg-muted text-muted-foreground rounded px-1 py-px text-[9px] font-medium uppercase tracking-wide">
            soon
          </span>
        )}
      </TooltipContent>
